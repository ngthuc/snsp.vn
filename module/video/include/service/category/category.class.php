 ...
 */
class Video_Service_Category_Category extends Phpfox_Service 
{
	private $_sOutput = '';
	...
	{		
		$sCacheId = $this->cache()->set('video_category_browse' . ($iCategoryId === null ? '' : '_' . $iCategoryId));
		...
		{					
			...
			{
				$aCategories[$iKey]['url'] = Phpfox::permalink('video.category', $aCategory['category_id'], $aCategory['name']);
				...
				//if ($sCategory === null)
				{
					...
					{
						$aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::permalink('video.category', $aSubCategory['category_id'], $aSubCategory['name']);
					}
				}
			}
	...
	public function get()
	{
		$sCacheId = $this->cache()->set('video_category_display_' . $this->_sDisplay . '_' . Phpfox::getLib('locale')->getLangId());
	...
	{		
		$sCacheId = $this->cache()->set('video_parent_breadcrumb_' . md5($sCategory));
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		{			
			...
			{				
				$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::permalink('video.category', $aCategory['category_id'], $aCategory['name']));
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::permalink('video.category', $aCategories[0]['category_id'], $aCategories[0]['name']));
		}
	...
	{
		$sCacheId = $this->cache()->set('video_category_childern_' . $sCategory);
	...
	{
		$sCacheId = $this->cache()->set('video_category_parent_' . $sCategory);
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('video.service_category_category__call'))
		{
			return eval($sPlugin);
		}
	...
	private function _get($iParentId, $iActive = null)
	{
		...
		if (count($aCategories))
		{
			...
			else 
			{
				...
				$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? Phpfox::getPhrase('video.select') : Phpfox::getPhrase('video.select_a_sub_category')) . ':</option>' . "\n";
			}
	...
	private function _getParentsUrl($iParentId, $bPassName = false)
	{
		...
		if ($iCnt === 1)
		{
			...
			$sCacheId = $this->cache()->set('video_category_url' . ($bPassName ? '_name' : '') . '_' . $iParentId);
