 ...
 */
class Marketplace_Service_Category_Category extends Phpfox_Service 
{
	private $_sOutput = '';
	...
	public function getForBrowse($iCategoryId = null)
	{
		...
		{					
			...
			{
				$aCategories[$iKey]['url'] = Phpfox::permalink('marketplace.category', $aCategory['category_id'], $aCategory['name']);
	...
	public function get()
	{
		$sCacheId = $this->cache()->set('marketplace_category_display_' . $this->_sDisplay);
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		{			
			...
			{				
				$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::permalink('marketplace.category', $aCategory['category_id'], $aCategory['name']));
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::permalink('marketplace.category', $aCategories[0]['category_id'], $aCategories[0]['name']));
		}				
