	...
	<form method="post" action="#" id="js_activity_feed_form" enctype="multipart/form-data">
		...
		<div class="activity_feed_form_holder">		
			...
			<div class="global_attachment_holder_section" id="global_attachment_status" style="display:block;">
				...
				<textarea {if Phpfox::getService('profile')->timeline()}style="height:31px;"{else}style="height:16px;"{/if}{if isset($aPage)} id="pageFeedTextarea" {else} {if isset($aEvent)} id="eventFeedTextarea" {else} {if isset($bOwnProfile) && $bOwnProfile == false}id="profileFeedTextarea" {/if}{/if}{/if} cols="60" rows="8" name="val[user_status]" {if isset($aFeedCallback.module) || defined('PHPFOX_IS_USER_PROFILE')} placeholder="{phrase var='feed.write_something'}" title="{phrase var='feed.write_something'}" {else} placeholder="{phrase var='feed.what_s_on_your_mind'}" title="{phrase var='feed.what_s_on_your_mind'}"{/if}>{if isset($aFeedCallback.module) || defined('PHPFOX_IS_USER_PROFILE')}{phrase var='feed.write_something'}{else}{phrase var='feed.what_s_on_your_mind'}{/if}</textarea>
