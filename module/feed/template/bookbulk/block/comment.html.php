		...
		<div class="js_feed_comment_form" {if isset($sFeedType) &&  $sFeedType == 'view'} id="js_feed_comment_form_{$aFeed.feed_id}"{/if}>
			...
			<div class="{if isset($sFeedType) &&  $sFeedType == 'view'} feed_item_view{/if} comment_mini comment_mini_end">
				<form method="post" action="#" class="js_comment_feed_form">				
					...
					{img user=$aGlobalUser suffix='_50_square' max_width='32' max_height='32' class="newLayPic"}
					</div>				
					{/if}	
					<div class="{if isset($sFeedType) &&  $sFeedType == 'view'}comment_mini_content {/if}comment_mini_textarea_holder">
						...
						<textarea placeholder="{phrase var='feed.write_a_comment'}" cols="60" rows="4" name="val[text]" onkeydown="if (event.keyCode == 13 && !event.shiftKey) document.getElementById('BtnComment_{$aFeed.feed_id}').click()" class="js_comment_feed_textarea" id="js_feed_comment_form_textarea_{$aFeed.feed_id}">{if isset($sFeedType) && $sFeedType == 'view' && Phpfox::getUserParam('comment.wysiwyg_on_comments') && Phpfox::getParam('core.wysiwyg') == 'tiny_mce'}{else}{phrase var='feed.write_a_comment'}{/if}</textarea>
                                        <div class="js_feed_comment_process_form">{phrase var='feed.adding_your_comment'}{img theme='ajax/add.gif'}</div>
					</div>
					<div class="feed_comment_buttons_wrap" style="{if Phpfox::isMobile()}display:block;{else}display:none;{/if}">
						<div class="js_feed_add_comment_button t_right">
							<input type="submit" {if Phpfox::isMobile()}{else}id="BtnComment_{$aFeed.feed_id}"{/if} value="{phrase var='feed.comment'}" class="button button_set_off" {if Phpfox::isMobile()}{else}style="display:none;"{/if}/>								
						</div>								
					</div>			
