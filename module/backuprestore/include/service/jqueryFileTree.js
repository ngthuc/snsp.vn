...
if(jQuery) (function($){
	$.extend($.fn, { 
		fileTree: function(o) {
			...
			if( o.script === undefined ) o.script = '../../../module/backuprestore/Classes/jqueryFileTree.php';
			...
			$(this).each( function() {  
				function showTree(c, t) {
                                      // if (get_checkbox_state(c.find('.checkbox')) === INCLUDED) {
				...
				function bindTree(t) {
					$(t).find('LI A').bind(o.folderEvent, function() { 
						if( $(this).parent().hasClass('directory') ) {
							if( $(this).parent().hasClass('collapsed') ) {
								...
								// Collapse
                                                                //console.log('Close expand directory');
					...
					$(t).find('ul').find('.checkbox').bind('click', function() {
						if (get_checkbox_state(this) === PARTIAL) //cheked childrens
							return;
						 checkbox_click(this);
					});
			...
			function set_checkbox_state(check_box, new_state) {
                                //console.log('set state'+new_state);
				...
				$('.checkbox').each(function () {
					if (clicked_parent_dir != o.root) {
						...
						if (parent_dir === clicked_parent_dir) {
							...
							if (state === PARTIAL || state === INCLUDED) {
								checked_count++;
							}
							total++;
						}
			...
			function checkbox_click(clicked) {
				...
				var state = get_checkbox_state(clicked) === INCLUDED ? EXCLUDED : INCLUDED;
                                //console.log('chekbox click set state to=>'+state);
				if (state === INCLUDED)
					collapse(clicked);
