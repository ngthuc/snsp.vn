 ...
 * @version 		$Id: category.class.php 1496 2010-03-05 17:15:05Z Raymond_Benc $
 */
class Group_Service_Category_Category extends Phpfox_Service 
{
	private $_sOutput = '';
	...
	{		
		$sCacheId = $this->cache()->set('group_category_browse' . ($sCategory === null ? '' : '_' . md5($sCategory)));
		...
		{		
			...
			foreach ($aCategories as $iKey => $aCategory)
			{
				if ($sCategory === null)
				{				
					$aCategories[$iKey]['url'] = Phpfox::getLib('url')->makeUrl('group.category', $aCategory['name_url']);
				}
				else 
				{
					...
					$aCategories[$iKey]['url'] = Phpfox::getLib('url')->makeUrl('group.category', $aParentCache);
				}
				...
				//if ($sCategory === null)
				{
					...
					{
						$aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::getLib('url')->makeUrl('group.category', array($aCategory['name_url'], $aSubCategory['name_url']));
					}
				}
			}
	...
	public function get()
	{
		$sCacheId = $this->cache()->set('group_category_display_' . $this->_sDisplay);
	...
	{		
		$sCacheId = $this->cache()->set('group_parent_breadcrumb_' . md5($sCategory));
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		if ($aCategories === null)
		{
			$aCategories = $this->database()->select('pc.parent_id, pc.category_id, pc.name, pc.name_url')
				->from(Phpfox::getT('group_category_data'), 'pcd')
				->join($this->_sTable, 'pc', 'pc.category_id = pcd.category_id')
				->where('pcd.group_id = ' . (int) $iId)
		...
		{			
			...
			{				
				if ($aCategory['parent_id'] > 0)
				{
					...
					$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::getLib('url')->makeUrl('group.category', $aCache));
				}				
				else 
				{
					$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::getLib('url')->makeUrl('group.category', $aCategory['name_url']));
				}				
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::getLib('url')->makeUrl('group.category', $aCategories[0]['name_url']));
		}
	...
	public function getCategoryIds($iId)
	{
		$aCategories = $this->database()->select('category_id')
			->from(Phpfox::getT('group_category_data'))
	...
	{
		$sCacheId = $this->cache()->set('group_category_childern_' . $sCategory);
	...
	{
		$sCacheId = $this->cache()->set('group_category_parent_' . $sCategory);
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('group.service_category_category__call'))
		{
			return eval($sPlugin);
		}
	...
	private function _get($iParentId, $iActive = null)
	{
		...
		if (count($aCategories))
		{
			...
			else 
			{
				...
				$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? Phpfox::getPhrase('group.select') : Phpfox::getPhrase('group.select_a_sub_category')) . ':</option>' . "\n";
			}
	...
	private function _getParentsUrl($iParentId, $bPassName = false)
	{
		...
		if ($iCnt === 1)
		{
			...
			$sCacheId = $this->cache()->set('group_category_url' . ($bPassName ? '_name' : '') . '_' . $iParentId);
