...
<?php echo Phpfox::getLib('phpfox.image.helper')->display(array('user' => $this->_aVars['aGlobalUser'],'suffix' => '_50_square','max_width' => '32','max_height' => '32','class' => "newLayPic")); ?>
					</div>				
<?php endif; ?>
					<div class="<?php if (isset ( $this->_aVars['sFeedType'] ) && $this->_aVars['sFeedType'] == 'view'): ?>comment_mini_content <?php endif; ?>comment_mini_textarea_holder">
						...
						<textarea placeholder="<?php echo Phpfox::getPhrase('feed.write_a_comment'); ?>" cols="60" rows="4" name="val[text]" onkeydown="if (event.keyCode == 13 && !event.shiftKey) document.getElementById('BtnComment_<?php echo $this->_aVars['aFeed']['feed_id']; ?>').click()" class="js_comment_feed_textarea" id="js_feed_comment_form_textarea_<?php echo $this->_aVars['aFeed']['feed_id']; ?>"><?php if (isset ( $this->_aVars['sFeedType'] ) && $this->_aVars['sFeedType'] == 'view' && Phpfox ::getUserParam('comment.wysiwyg_on_comments') && Phpfox ::getParam('core.wysiwyg') == 'tiny_mce'):  else:  echo Phpfox::getPhrase('feed.write_a_comment');  endif; ?></textarea>
					...
					<div class="feed_comment_buttons_wrap" style="<?php if (Phpfox ::isMobile()): ?>display:block;<?php else: ?>display:none;<?php endif; ?>">
						<div class="js_feed_add_comment_button t_right">
							<input type="submit" <?php if (Phpfox ::isMobile()):  else: ?>id="BtnComment_<?php echo $this->_aVars['aFeed']['feed_id']; ?>"<?php endif; ?> value="<?php echo Phpfox::getPhrase('feed.comment'); ?>" class="button button_set_off" <?php if (Phpfox ::isMobile()):  else: ?>style="display:none;"<?php endif; ?>/>								
						</div>								
					</div>			
