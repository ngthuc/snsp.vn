...
<?php endif; ?>
		<div class="activity_feed_form_holder">		
			...
			<div class="global_attachment_holder_section" id="global_attachment_status" style="display:block;">
				...
				<textarea <?php if (Phpfox ::getService('profile')->timeline()): ?>style="height:31px;"<?php else: ?>style="height:16px;"<?php endif;  if (isset ( $this->_aVars['aPage'] )): ?> id="pageFeedTextarea" <?php else: ?> <?php if (isset ( $this->_aVars['aEvent'] )): ?> id="eventFeedTextarea" <?php else: ?> <?php if (isset ( $this->_aVars['bOwnProfile'] ) && $this->_aVars['bOwnProfile'] == false): ?>id="profileFeedTextarea" <?php endif;  endif;  endif; ?> cols="60" rows="8" name="val[user_status]" <?php if (isset ( $this->_aVars['aFeedCallback']['module'] ) || defined ( 'PHPFOX_IS_USER_PROFILE' )): ?> placeholder="<?php echo Phpfox::getPhrase('feed.write_something'); ?>" title="<?php echo Phpfox::getPhrase('feed.write_something'); ?>" <?php else: ?> placeholder="<?php echo Phpfox::getPhrase('feed.what_s_on_your_mind'); ?>" title="<?php echo Phpfox::getPhrase('feed.what_s_on_your_mind'); ?>"<?php endif; ?>><?php if (isset ( $this->_aVars['aFeedCallback']['module'] ) || defined ( 'PHPFOX_IS_USER_PROFILE' )):  echo Phpfox::getPhrase('feed.write_something');  else:  echo Phpfox::getPhrase('feed.what_s_on_your_mind');  endif; ?></textarea>
