...
{$('#'+Editor.getId()).val(mValue);}
 /* module/event/static/jscript/add.js */$Behavior.addNewEvent=function()
{$('.js_event_change_group').click(function()
{if($(this).parent().hasClass('locked'))
{return false;}
aParts=explode('#',this.href);$('.js_event_block').hide();$('#js_event_block_'+aParts[1]).show();$(this).parents('.header_bar_menu:first').find('li').removeClass('active');$(this).parent().addClass('active');$('#js_event_add_action').val(aParts[1]);});$('.js_mp_category_list').change(function()
...
{$('#js_mp_holder_'+this.id.replace('js_mp_id_','')).hide();this.value='';}});$('#js_mp_holder_'+$(this).val()).show();});}
 /* static\jscript\progress.js */var oProgressBar={};var sImages="";var bIsHTML5ProgressUpload=false;if(window.FormData!==undefined)
{bIsHTML5ProgressUpload=true;}
$Core.loadStaticFile(getParam('sProgressCssFile'));$Behavior.checkHTML5UploadSupport=function(){if(bIsHTML5ProgressUpload){$('.js_upload_button_link').hide();}}
function completeProgress()
{iTotalUploadedFiles--;iTotalImagesToBeUploaded--;if(function_exists('plugin_completeProgress'))
{plugin_completeProgress();}
$('.js_uploader_files_cache').remove();$('.js_uploader_files_input').attr('disabled',false);$('#js_progress_outer').hide();$Core.loadInit();if(bIsHTML5ProgressUpload&&iTotalUploadedFiles<=0&&iTotalImagesToBeUploaded<=0)
...
{iUploaded=0;iExtFailed=0;if(bIsHTML5ProgressUpload){}else{$('.js_uploader_files_input').each(function()
{if(!empty(this.value))
{iUploaded++;if(isset(oProgressBar['valid_file_ext']))
{sExt=this.value.split('.').pop().toLowerCase();if($.inArray(sExt,oProgressBar['valid_file_ext'])==-1)
{iExtFailed++;}}}});}
if(iExtFailed>0)
{sExts='';for(iExt in oProgressBar['valid_file_ext'])
{if(iExt>0)
{sExts+=', ';}
sExts+=oProgressBar['valid_file_ext'][iExt];}
alert($('<div/>').html(oTranslations['core.not_a_valid_file_extension_we_only_allow_ext'].replace('{ext}',sExts)).text());return false;}
if(!bForceImage&&!iUploaded)
{return bForm;}
if(!iUploaded)
{alert($('<div/>').html(oTranslations['core.select_a_file_to_upload']).text());return false;}
$('.error_message').remove();$('#js_progress_cache_holder').remove();if(bForm)
{$(oProgressBar['holder']).hide();$(oProgressBar['holder']+'_loading').height($(oProgressBar['holder']).innerHeight());$(oProgressBar['holder']+'_loading').show();return true;}
return false;}
...
{alert(oTranslations['core.select_a_file_to_upload']);return false;}
...
{$(this).addClass('js_uploader_files_cache').hide();});sInput='';for(i=1;i<=oProgressBar['total'];i++)
{sInput+='<div class="js_uploader_files"><input type="file" name="'+oProgressBar['file_id']+'" size="30" class="js_uploader_files_input" disabled="disabled" onchange="addMoreToProgressBar();" /></div>'+"\n";}
$('#js_uploader_files_outer').append(sInput);return true;}
var iNewInputBars=0;function addMoreToProgressBar()
{iNewInputBars++;if((iNewInputBars+oProgressBar['total'])>oProgressBar['max_upload'])
{iNewInputBars--;return false;}
$('.js_uploader_files_input').each(function()
{if(empty(this.value))
{iNewInputBars--;$(this).parent().remove();}});$('#js_uploader_files_outer').append('<div class="js_uploader_files" id="js_new_add_input_'+iNewInputBars+'"><input type="file" name="'+oProgressBar['file_id']+'" class="js_uploader_files_input" size="30" onchange="addMoreToProgressBar();" /></div>'+"\n");return false;}
function removeMoreToProgressBar(iId)
{iNewInputBars--;$('#js_new_add_input_'+iId).remove();return false;}
var iTotalImagesToBeUploaded=0;$Core.progressBarInit=function()
{if(!isset(oProgressBar['html5upload'])||(isset(oProgressBar['html5upload'])&&!oProgressBar['html5upload'])){bIsHTML5ProgressUpload=false;}
if($(oProgressBar['uploader']).length>0)
{$(oProgressBar['progress_id']).html('<div id="js_progress_outer" style="width:300px;"><div id="js_progress_inner"><span id="js_progress_percent_value">0</span>/100%</div></div>');sInput='<div id="js_uploader_files_outer">';if(bIsHTML5ProgressUpload)
...
sInput+='</div>';var iDivHeight=$(oProgressBar['holder']).innerHeight();$(oProgressBar['holder']).after('<div id="js_progress_cache_loader" style="height:'+(iDivHeight<=0?'200':iDivHeight)+'px; display:none;"></div>');sInput+='<iframe id="'+oProgressBar['frame_id']+'" name="'+oProgressBar['frame_id']+'" height="500" width="500" frameborder="1" style="display:none;"></iframe>';$(oProgressBar['uploader']).html(sInput);$.ajaxCall('user.checkSpaceUsage','holder='+oProgressBar['holder'].replace('#',''),'GET');if(bIsHTML5ProgressUpload){$('.js_uploader_files_input')[0].addEventListener("change",function(e){var files=e.target.files||e.dataTransfer.files;iTotalImagesToBeUploaded=files.length;for(var i=0,f;f=files[i];i++){if(i>=oProgressBar['max_upload']){break;}
if(isset(oProgressBar['valid_file_ext']))
{sExt=f.name.split('.').pop().toLowerCase();if($.inArray(sExt,oProgressBar['valid_file_ext'])==-1)
{sExts='';for(iExt in oProgressBar['valid_file_ext'])
{if(iExt>0)
{sExts+=', ';}
sExts+=oProgressBar['valid_file_ext'][iExt];}
alert($('<div/>').html(oTranslations['core.not_a_valid_file_extension_we_only_allow_ext'].replace('{ext}',sExts)).text());break;}}
ParseFile(f,i);UploadFile(f,i);}},false);}}};function ParseFile(file,iCnt){$(oProgressBar['holder']).after("<div id=\"js_tmp_upload_"+iCnt+"\" class=\"js_tmp_upload_bar\"><div class=\"js_tmp_upload_bar_content\">"+file.name+"</div><div class=\"js_tmp_upload_bar_upload\"></div></div>").hide();if(file.type.indexOf("image")==0){var reader=new FileReader();reader.onload=function(e){$('#js_tmp_upload_'+iCnt).prepend('<div style="position:absolute; right:5px; top:2px; z-index:2;"><img src="'+e.target.result+'" style="max-width:25px; max-height:25px;" /></div>');}
reader.readAsDataURL(file);}}
var iTotalUploadedFiles=0;var sCustomMessageString='';function UploadFile(file,iCnt){var xhr=new XMLHttpRequest();if(xhr.upload){xhr.upload.addEventListener("progress",function(e){var pc=parseInt((e.loaded/e.total*100));$('#js_tmp_upload_'+iCnt+'').find('.js_tmp_upload_bar_upload').width(pc+'%').show();if(pc===100&&iTotalImagesToBeUploaded===(iCnt+1)){sCustomMessageString=oTranslations['photo.upload_complete_we_are_currently_processing_the_photos'];tb_show(oTranslations['photo.photo_uploads'],$.ajaxBox('core.message','height=100&width=400'));console.log(oTranslations['photo.photo_uploads']);console.log(oTranslations['photo.upload_complete_we_are_currently_processing_the_photos']);}},false);xhr.onreadystatechange=function(){if(xhr.readyState==4){if(xhr.status==200){iTotalUploadedFiles++;eval(xhr.responseText);}else{$(oProgressBar['holder']).show();eval(xhr.responseText.replace(/window.parent./g,''));$('.js_tmp_upload_bar').remove();}}}
xhr.open("POST",$(oProgressBar['holder']).find('form').attr('action'),true);xhr.setRequestHeader("X-FILENAME",encodeURIComponent(file.name));xhr.setRequestHeader("X-POST-FORM",$(oProgressBar['holder']).find('form').getForm());xhr.send(file);}}
