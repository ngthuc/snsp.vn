...
{$bUserToolTipIsHover=false;oCloseObject=$(this).parent().attr('id').replace('js_user_name_link_','');setTimeout('$Core.closeUserToolTip(\''+oCloseObject+'\');',$iUserToolTipWaitTime);});};
 /* static\jscript\selector.js */$Behavior.selector=function()
...
{this.checked=bStatus;if(bStatus)
{iCnt++;}});if(iCnt)
{$('#js_selector_count').html(iCnt);$('#js_selector_action').show();}
else
{$('#js_selector_action').hide();}});$('.checkbox').click(function()
{var iCnt=0;$("input:checkbox").each(function()
{if(this.checked)
{iCnt++;$('.js_selector_class_'+this.value).addClass('isSelected');}
else
{$('.js_selector_class_'+this.value).removeClass('isSelected');}});if(iCnt)
{$('#js_selector_count').html(iCnt);$('#js_selector_action').show();$('#js_action_selector').attr('disabled',false);$('.js_select_all_action').show();$('.js_select_unall_button').show();$('.js_select_all_button').hide();}
else
{$('#js_selector_action').hide();$('#js_action_selector').attr('disabled',true);}});$('.js_select_all_button').click(function()
{$("input:checkbox").each(function()
{$('.js_selector_class_'+this.value).addClass('isSelected');this.checked=true;});$('.js_select_all_action').show();$('.js_select_unall_button').show();$(this).hide();});$('.js_select_unall_button').click(function()
{$("input:checkbox").each(function()
{$('.js_selector_class_'+this.value).removeClass('isSelected');this.checked=false;});$('.js_select_all_action').hide();$('.js_select_all_button').show();$(this).hide();});$('.js_selector').change(function()
{if(this.value=='all')
{$("input:checkbox").each(function()
{$('.js_selector_class_'+this.value).addClass('isSelected');this.checked=true;});}
else if(this.value=='none')
{$("input:checkbox").each(function()
{$('.js_selector_class_'+this.value).removeClass('isSelected');this.checked=false;});}
else if(this.value=='')
{return false;}
...
{$("input:checkbox").each(function()
{$('.js_selector_class_'+this.value).removeClass('isSelected');this.checked=false;});aIds=$('#js_selector_'+this.value).val().split(',');for(i=0;i<aIds.length;i++)
{if(aIds[i]=='')
{continue;}
$('.js_selector_class_'+aIds[i]).addClass('isSelected');$('#js_selector_checkbox_'+aIds[i]).attr('checked',true);}}});};
 /* module/mail/static/jscript/mail.js */$Behavior.mailView=function()
{if($Core.exists('#js_mail_textarea')){$('#js_mail_textarea #message').keydown(function(){$Core.resizeTextarea($(this));});}
if($Core.exists('.mail_thread_form_holder')){$('.mail_thread_form_holder').width($('#site_content').width());}
if($Core.exists('#js_mail_thread_view_more')){$(window).scroll(function(){if(isScrolledIntoView('#mail_threaded_new_message_scroll')){$('.mail_thread_form_holder').addClass('not_fixed');}
else{$('.mail_thread_form_holder').removeClass('not_fixed');}});$.scrollTo('.is_last_message',800);}
$('#js_mail_thread_view_more').click(function(){$(this).hide();$('.mail_thread_holder').removeClass('is_last_message');$('#feed_view_more_loader').show();$.ajaxCall('mail.viewMoreThreadMail','page='+$('#js_mail_thread_current_cnt').html()+'&thread_id='+$(this).attr('rel'),'GET');return false;});}
$Core.addThreadMail=function(oObj){var sContent=Editor.getContent();Editor.setContent('');$(oObj).ajaxCall('mail.addThreadMail','val[message]='+encodeURIComponent(sContent));$(oObj).find('.js_attachment_list').hide();$(oObj).find('.js_attachment:first').val('');}
$Core.mailThreadReset=function(){$('#feed_view_more_loader').hide();$('#js_mail_thread_view_more').show();var iTotal=parseInt($('#js_mail_thread_current_cnt').html());$('#js_mail_thread_current_cnt').html(iTotal+1);$.scrollTo('.is_last_message:first');}
